 ...
 *
 *  ==================================================
 *  Copyright 2003-2018 SoftChalk LLC
 *  All Rights Reserved.
 *
 *  http://www.softchalk.com
 *  ==================================================
 *
 *  File date: September 21, 2018
 */
...
$(document).ready(function () {
    ...
    wrapMediaBasedIframes('iframe',['nothingyet']);
    wrapMediaBasedIframes('embed',['nothingyet']);
    wrapMediaBasedIframes('video',['nothingyet']);
 ...
 * @param {*} extentsions
 */
function wrapMediaBasedIframes(tag, extentsions){
    var collection = document.querySelectorAll(tag);
    var supportedTypes = [".avi",".mpg",".mpeg",".mp4", ".m4v", ".mov",".wmv",".ogg"];

    for(var item in collection){
        var isVideo = false;
        if(tag == 'iframe' && !isNaN(item)){
            isVideo =isYoutubeVideo(collection, item);
        }
        if(tag == 'video'){
            isVideo = true;
        }
        if(tag == 'embed'&& !isNaN(item)){
            isVideo = isVideoEmbed(collection, item, supportedTypes);
        }
        if(!isNaN(item) && isVideo == true){
...
/**
 * @function isYoutubeVideo: loop through items and check if the source has '.youtube.com'
 * @param {*} collection: an array of items we look through
 * @param {*} item : the index for that particular item
 */
function isYoutubeVideo (collection, item){
    var isVideo
...
/**
 * @function isVideoEmbed : checks if embed is a supported video type.  if it is we return true
 * @param {*} collection
 * @param {*} item
 * @param {*} supportedTypes
 * @param {*} source
 */
function isVideoEmbed(collection, item, supportedTypes){
    var source = collection[item].getAttribute('src');
    var isVideo = false;
    var lastIndexOfPeriod = source.lastIndexOf('.');
    var extension = source.substring(lastIndexOfPeriod);
    var isVideo = (function(){
        if (supportedTypes.indexOf(extension.substring(0,4)) > -1 || extension.indexOf('.mpeg') > -1){
            return true;
        }
        else{
            return isVideo
...
function initPopovers() {
    ...
    // Check if it is iOS, add cursor to body to allow click to close on mobile ios
